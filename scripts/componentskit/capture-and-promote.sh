...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-componentskit/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-componentskit-port"
...
while IFS='|' read -r folder slug; do
  echo "[componentskit-port] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-componentskit-build.log 2>&1
...
echo "[componentskit-port] capture and promotion completed"
