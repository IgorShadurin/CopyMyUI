...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-alerttoast-variety/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-alerttoast-variety"
...
while IFS='|' read -r folder slug; do
  echo "[alerttoast-variety] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-alerttoast-variety-build.log 2>&1
...
echo "[alerttoast-variety] capture and promotion completed"
