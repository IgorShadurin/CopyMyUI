...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-elegantcalendar/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-elegantcalendar-port"
...
while IFS='|' read -r folder slug; do
  echo "[elegantcalendar-port] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-elegantcalendar-build.log 2>&1
...
echo "[elegantcalendar-port] capture and promotion completed"
