...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat/dockui-port-manifest.json"
...
DERIVED="/tmp/copymyui-dockui-port"
...
while IFS='|' read -r folder slug; do
  echo "[dockui-port] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-dockui-build.log 2>&1
...
echo "[dockui-port] capture and promotion completed"
