...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-alerttoast-variety-b2/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-alerttoast-variety-b2"
...
while IFS='|' read -r folder slug; do
  echo "[alerttoast-variety-b2] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-alerttoast-variety-b2-build.log 2>&1
...
echo "[alerttoast-variety-b2] capture and promotion completed"
