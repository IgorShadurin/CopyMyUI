...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-detailspro-variety/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-detailspro-variety"
...
while IFS='|' read -r folder slug; do
  echo "[detailspro-variety] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-detailspro-variety-build.log 2>&1
...
echo "[detailspro-variety] capture and promotion completed"
