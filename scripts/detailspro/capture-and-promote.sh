...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-detailspro/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-detailspro-port"
...
while IFS='|' read -r folder slug; do
  echo "[detailspro-port] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-detailspro-build.log 2>&1
...
echo "[detailspro-port] capture and promotion completed"
