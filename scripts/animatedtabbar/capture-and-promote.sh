...
PORT_MANIFEST="$REPO_ROOT/public/uploads/copycat-animatedtabbar/source-port-manifest.json"
...
DERIVED="/tmp/copymyui-animatedtabbar-port"
...
while IFS='|' read -r folder slug; do
  echo "[animatedtabbar-port] processing $folder ($slug)"
  ...
  xcodebuild \
    ...
    build >/tmp/copymyui-animatedtabbar-build.log 2>&1
...
echo "[animatedtabbar-port] capture and promotion completed"
